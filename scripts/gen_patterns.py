...
write("pages/dataviz/index.html", page(
    ...
    "Diretrizes pra visualizações de dados: anatomia de chart, tipos canônicos, paletas categóricas, sequenciais e divergentes.",
    "".join([
        sec("overview", "visão geral", "01",
            '<p class="t-body-02 t-secondary mb-05" style="max-width:720px">O sistema de dataviz é beta. Diretrizes mínimas pra garantir consistência visual e acessibilidade entre charts dos diferentes produtos.</p>' +
            '<div class="resource-cards">' +
            '<a class="resource-card" href="anatomy.html"><div class="meta">foundations</div><h4>chart anatomy</h4><p>Componentes universais de qualquer chart.</p><span class="cta">explorar</span></a>' +
            '<a class="resource-card" href="types.html"><div class="meta">tipos</div><h4>chart types</h4><p>Bar, line, area, pie, scatter — quando usar cada.</p><span class="cta">explorar</span></a>' +
            '<a class="resource-card" href="colors.html"><div class="meta">paleta</div><h4>color palettes</h4><p>Categóricas, sequenciais, divergentes.</p><span class="cta">explorar</span></a>' +
            '</div>'),
    ]),
    tags=[{"cls":"tag--script","label":"beta"}],
    toc=[{"id":"overview","label":"Visão geral"}],
))

write("pages/dataviz/anatomy.html", page(
    "dv-anatomy", "Chart anatomy",
    '<a href="../../index.html">home</a><span class="sep">/</span><a href="index.html">dataviz</a><span class="sep">/</span>anatomy',
    "Componentes universais de qualquer chart: título, eixos, legenda, séries, tooltip, área de plot.",
    "".join([
        sec("parts", "componentes", "01",
            demo('<div style="max-width: 560px"><div class="t-label-01 mb-04" style="color: var(--text-helper)">// vendas por mês · 2026</div><div style="background: var(--layer-01); border: 1px solid var(--border-subtle-00); padding: var(--spacing-05); height: 200px; position: relative"><div style="height: 100%; display: flex; align-items: flex-end; gap: 8px;">' + ''.join(f'<div style="background: var(--ch-code); flex: 1; height: {30+(i*7)%70}%"></div>' for i in range(12)) + '</div></div><div class="row mt-04" style="gap: var(--spacing-05); font: var(--code-01); color: var(--text-helper)"><span><span style="display:inline-block;width:8px;height:8px;background:var(--ch-code);margin-right:4px"></span>vendas</span></div></div>'),
        ),
        sec("checklist", "elementos obrigatórios", "02",
            checklist([
                "Título descritivo acima",
                "Eixos com labels claros (incluir unidade)",
                "Legenda quando há 2+ séries",
                "Tooltip on hover com valor exato",
                "Gridlines sutis em --border-subtle-00",
                "Cores da paleta dataviz (não as cores brand cruas)",
            ])),
        sec("a11y", "acessibilidade", "03",
            checklist([
                "Texto alternativo descrevendo trend",
                "Tabela de dados como fallback semântico",
                'role="img" no SVG do chart',
                "Cores distinguíveis em daltonismo (testar)",
                "Padrões visuais (hachura) como alternativa à cor",
            ])),
    ]),
    toc=[{"id":"parts","label":"Componentes"},{"id":"checklist","label":"Elementos obrigatórios"},{"id":"a11y","label":"Acessibilidade"}],
))

write("pages/dataviz/types.html", page(
    "dv-types", "Chart types",
    '<a href="../../index.html">home</a><span class="sep">/</span><a href="index.html">dataviz</a><span class="sep">/</span>types',
    "Cada tipo de chart responde a uma pergunta diferente. Escolha errada de chart cria mais confusão que clareza.",
    "".join([
        sec("matrix", "matriz de decisão", "01",
            table(["pergunta","chart","exemplo"], [
                ["Comparação entre categorias","Bar chart","vendas por produto"],
                ["Tendência ao longo do tempo","Line chart","usuários por mês"],
                ["Composição (parte do todo)","Pie / donut (até 5 fatias)","share por segmento"],
                ["Distribuição","Histograma","idade dos usuários"],
                ["Correlação","Scatter plot","preço × demanda"],
                ["Stacked composition + tempo","Stacked area","receita por canal × mês"],
                ["Hierarquia","Treemap","gastos por categoria"],
            ])),
        sec("examples", "exemplos visuais", "02",
            '<div class="grid-2">' +
            '<div class="demo">' + 'BAR ' + ''.join(f'<div style="display:inline-block;width:20px;height:{20+(i*8)%50}px;background:var(--ch-code);margin:0 2px;vertical-align:bottom"></div>' for i in range(8)) + '</div>' +
            '<div class="demo">' + '<svg width="200" height="80" viewBox="0 0 200 80"><polyline points="10,60 40,40 70,50 100,20 130,30 160,15 190,25" stroke="#32FA96" stroke-width="2" fill="none"/></svg> LINE</div>' +
            '<div class="demo">' + '<svg width="80" height="80" viewBox="0 0 32 32"><circle cx="16" cy="16" r="14" fill="#32FA96"/><path d="M16 16 L16 2 A14 14 0 0 1 28 24 Z" fill="#AA78E6"/><path d="M16 16 L28 24 A14 14 0 0 1 16 30 Z" fill="#3C433C"/></svg> PIE</div>' +
            '<div class="demo">' + '<svg width="200" height="80" viewBox="0 0 200 80">' + ''.join(f'<circle cx="{(i*23)%180+10}" cy="{(i*17)%60+10}" r="3" fill="#32FA96"/>' for i in range(20)) + '</svg> SCATTER</div>' +
            '</div>'),
        sec("avoid", "evitar", "03",
            do_dont(
                ["Bar pra comparar 2-30 categorias","Line pra tendência temporal","Stacked area pra composições ao longo do tempo","Tabela quando precisão > tendência"],
                ["Pie com 6+ fatias (vira mosaico ilegível)","Donut com label dentro (raramente cabe)","3D charts (distorcem percepção)","Dual-axis (confuso, propenso a interpretação errada)"],
            )),
    ]),
    toc=[{"id":"matrix","label":"Matriz de decisão"},{"id":"examples","label":"Exemplos visuais"},{"id":"avoid","label":"Evitar"}],
))

write("pages/dataviz/colors.html", page(
    "dv-colors", "Color palettes",
    '<a href="../../index.html">home</a><span class="sep">/</span><a href="index.html">dataviz</a><span class="sep">/</span>colors',
    "Paletas distintas pra dados categóricos (qualitativos), sequenciais (quantidades) e divergentes (extremos).",
    "".join([
        sec("categorical", "categórica · 8 cores", "01",
            '<p class="t-body-02 t-secondary mb-05" style="max-width:720px">Pra séries qualitativas (categorias sem ordem). Distinguíveis em daltonismo.</p>' +
            demo('<div style="display: flex; height: 64px;"><div style="flex:1; background:#32FA96"></div><div style="flex:1; background:#AA78E6"></div><div style="flex:1; background:#D79B2E"></div><div style="flex:1; background:#B3D9FE"></div><div style="flex:1; background:#FF9ECF"></div><div style="flex:1; background:#3C433C"></div><div style="flex:1; background:#5CFFAA"></div><div style="flex:1; background:#D3BAF5"></div></div><p class="t-helper mt-04">code · purple · script · sec-blue · sec-pink · dos · code-light · purple-light</p>')),
        sec("sequential", "sequencial · 7 steps", "02",
            '<p class="t-body-02 t-secondary mb-05" style="max-width:720px">Pra dados ordenados (intensidade, frequência). Gradiente do claro ao escuro.</p>' +
            demo('<div style="display: flex; height: 64px;"><div style="flex:1; background:#E1FFDE"></div><div style="flex:1; background:#C2F5DA"></div><div style="flex:1; background:#9BE5BF"></div><div style="flex:1; background:#5CFFAA"></div><div style="flex:1; background:#32FA96"></div><div style="flex:1; background:#24C775"></div><div style="flex:1; background:#0F5A33"></div></div>')),
        sec("divergent", "divergente · 7 steps", "03",
            '<p class="t-body-02 t-secondary mb-05" style="max-width:720px">Pra dados com ponto neutro e dois extremos (ex: variação positiva/negativa).</p>' +
            demo('<div style="display: flex; height: 64px;"><div style="flex:1; background:#DA1E28"></div><div style="flex:1; background:#FF8389"></div><div style="flex:1; background:#FFCCE3"></div><div style="flex:1; background:#F0F5F0"></div><div style="flex:1; background:#C2F5DA"></div><div style="flex:1; background:#5CFFAA"></div><div style="flex:1; background:#24C775"></div></div>')),
        sec("rules", "regras", "04",
            checklist([
                "Cor nunca é único portador de significado — sempre incluir label/legenda",
                "Testar em daltonismo (deuteranopia, protanopia)",
                "Para impressão preto-e-branco, garantir distinção por luminância",
                "Não misturar paleta categórica com sequencial no mesmo chart",
            ])),
    ]),
    toc=[{"id":"categorical","label":"Categórica"},{"id":"sequential","label":"Sequencial"},{"id":"divergent","label":"Divergente"},{"id":"rules","label":"Regras"}],
))

print("done · patterns + dataviz")
